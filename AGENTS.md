...
- `GekkoApp.sh` and `Gekko APP/GekkoApp.sh` are tracked, byte-identical launchers. If one changes, update the other and verify with `cmp -s`. Both accept `--gui` to launch the Tauri Control Center (preferring `target/release/gekkoapp-gui` and otherwise compiling `--features gui`).
- The shell fallback is not feature- or safety-equivalent to Rust: without Cargo/a package-local release binary it configures Chaotic AUR before the menu. Do not add features only to the fallback or use it for verification.
...
- `src/gui/mod.rs` owns the backend: `catalog_state` (local installed versions + detected environment, no network), `install_kito`, and `install_bauh` commands. They call the same `install_kito_plan`/`install_bauh` flows inside `tauri::async_runtime::spawn_blocking`, with `require_confirmation = false` (the GUI already confirmed). Progress streams to the frontend as `install://event` payloads `{kind: "log"|"progress", data}`.
